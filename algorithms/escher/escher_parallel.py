...
class PolicyNetwork(tf.keras.Model):
    ...
    def forward_weights(self, inputs):
        x, mask = inputs
    ...
    def call(self, inputs):
        ...
        """
        x, mask = self.forward_weights(inputs)
        x = tf.where(mask == 1, x, -10e20)
        x = self.softmax(x)
        return x


class PolicyNetworkSimple(tf.keras.Model):
    """Implements the policy network as an MLP.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.hidden = []
        for units in policy_network_layers:
            self.hidden.append(
                tf.keras.layers.Dense(units, kernel_initializer="he_normal")
            )
...
class ValueNetwork(tf.keras.Model):
    ...
    def __init__(
        self, input_size, val_network_layers, activation="leakyrelu", **kwargs
    ):
        super().__init__(**kwargs)
        self._input_size = input_size
        if activation == "leakyrelu":
...
class ESCHERSolver(policy.Policy):
    def __init__(
        ...
        num_traversals: int = 1000,
        num_val_fn_traversals: int = 1000,
        ...
        value_network_train_steps: int = 5000,
        ...
        **kwargs,
        ...
        if self._oshi_zumo:
            hist_state = get_oshi_hist_obs(
                self._root_node.observation_tensor(0), 0, starting_coins, 0
            )
        elif self._markov_soccer:
            hist_state = get_markov_soccer_hist_obs(
                self._root_node.observation_tensor(0), 0, 0
            )
        elif self._battleship:
        ...
        if self._save_average_policy_memories:
            if os.path.isdir(self._save_average_policy_memories):
                self._memories_tfrecordpath = os.path.join(
                    ...
                    "average_policy_memories.tfrecord",
                )
            else:
                os.makedirs(
        ...
        if create_worker_group:
            ...
            for _ in range(num_experience_workers):
                self.workers.append(
                    ray.remote(num_cpus=1)(ESCHERSolver).remote(
                        game,
                        create_worker_group=False,
    ...
    def _reinitialize_policy_network(self):
        ...
        with tf.device(self._train_device):
            self._policy_network = PolicyNetworkSimple(
    ...
    def _reinitialize_regret_network(self, player):
        ...
        with tf.device(self._train_device):
            self._regret_networks_train[player] = RegretNetwork(
                self._embedding_size, self._regret_network_layers, self._num_actions
            )
            self._optimizer_regrets[player] = tf.keras.optimizers.Adam(
                learning_rate=self._learning_rate
            )
            self._regret_train_step[player] = self._get_regret_train_graph(player)
    ...
    def _create_memories(self, memory_capacity):
        ...
        self._value_feature_description = {
            "hist_state": tf.io.FixedLenFeature(
                [self._value_embedding_size], tf.float32
            ),
            "iteration": tf.io.FixedLenFeature([1], tf.float32),
    ...
    def init_regret_net(self):
        ...
        for p in range(self._num_players):
            example_info_state = ray.get(
                self.workers[0].get_example_info_state.remote(p)
            )
            ...
            for w in self.workers:
                ray.get(
                    w.traverse_game_tree_n_times.remote(1, p, track_mean_squares=False)
                )
            self._init_main_regret_network(
                example_info_state, example_legal_actions_mask, p
            )
                ...
                if state.is_chance_node():
                    ...
                    if cur_player == player:
                        probs = self._policy_network(
                            (obs, legal_actions_mask), training=False
                        )
                        probs = probs.numpy()[0]
                        probs /= probs.sum()
    ...
    def pre_iteration(self):
        self.regret_losses = collections.defaultdict(list)
        self.value_losses = []
        str(datetime.now())
    ...
    def iteration(self):
        ...
        with tf.device(self._infer_device):
            ray.get(
                self.workers[0].traverse_game_tree_n_times.remote(
                    1, 0, track_mean_squares=False
                )
            )
            start = time.time()
            ...
            results = []
            for w in self.workers:
                result = w.traverse_game_tree_n_times.remote(
                ...
                result2 = w.traverse_game_tree_n_times.remote(
                    20,
                    0,
                    train_value=True,
                    track_mean_squares=False,
            ...
            self.value_losses.append(self._learn_value_network())
            self.sync_val_weights()
            print(self.value_losses[-1], "val loss")
            ...
            if (
                self._oshi_zumo
                or self._battleship
                or self._markov_soccer
                or self._dark_hex
                or self._phantom_ttt
            ):
                track_mse = False
            else:
                track_mse = True
            for p in range(self._num_players):
                ...
                self.regret_losses[p].append(self._learn_regret_network(p))
                if self._save_regret_networks:
                    os.makedirs(self._save_regret_networks, exist_ok=True)
        ...
        end_time = time.time()
        return {
            "reward_against_random": 0,
            "value_loss": self.value_losses[-1],
            "regret_loss": self.regret_losses,
            "global_step": num_nodes,
            "fps": (num_nodes) / (end_time - start),
        }
                ...
                for i in range(self._num_iterations + 1):
                    ...
                    if i % self._check_exploitability_every == 0:
                        ...
                        if self._save_policy_weights:
                            ...
                            model_path = (
                                save_path_model + "/policy_nodes_" + str(num_nodes)
                            )
                        ...
                        if self._play_against_random:
                            ...
                            avg_reward = self.play_n_games_against_random(
                                self._num_random_games
                            )
                            print(avg_reward, "Average reward against random\n")
                            if self._markov_soccer:
                                nfsp_avg_rew = self._run_n_episodes_against_nfsp(
                                    self._num_random_games
                                )
            ...
            features=tf.train.Features(
                feature={
                    ...
                    "action_probs": tf.train.Feature(
                        float_list=tf.train.FloatList(value=average_policy_action_probs)
                    ),
    ...
    def _serialize_value_memory(
        ...
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    "hist_state": tf.train.Feature(
                        float_list=tf.train.FloatList(value=hist_state)
                    ),
                    "iteration": tf.train.Feature(
                        float_list=tf.train.FloatList(value=[iteration])
                    ),
                    "samp_value": tf.train.Feature(
        ...
        if train_value:
            ...
            if on_policy_prob == 0 or expl == 0:
                if self._oshi_zumo:
                    ...
                    hist_state = get_markov_soccer_hist_obs(
                        state.observation_tensor(0), cur_player, last_action
                    )
                else:
                    hist_state = np.append(
    ...
    def _init_main_val_network(self, hist_state, legal_actions_mask):
        """TF-Graph to calculate regret matching."""
        self._val_network(
            (tf.expand_dims(hist_state, axis=0), legal_actions_mask), training=False
        )[0]
    ...
    def _sample_action_from_regret(self, state, player):
        ...
        if self._oshi_zumo:
            ...
            info_state = tf.constant(
                battleship_infostate_tensor(state, player), dtype=tf.float32
            )
        elif self._markov_soccer:
    ...
    def _estimate_value_from_hist(self, state, player, last_action=0):
        ...
        if self._oshi_zumo:
            ...
            hist_state = get_markov_soccer_hist_obs(
                state.observation_tensor(0), state.current_player(), last_action
            )
        else:
            hist_state = np.append(
    ...
    def action_probabilities(self, state):
        ...
        if self._oshi_zumo:
            ...
            info_state_vector = tf.constant(
                battleship_infostate_tensor(state, cur_player), dtype=tf.float32
            )
        elif self._markov_soccer:
            info_state_vector = tf.constant(
                state.observation_tensor(), dtype=tf.float32
            )
        else:
            info_state_vector = tf.constant(
        ...
        def train_step(info_states, regrets, iterations, masks, iteration):
            ...
            with tf.GradientTape() as tape:
                ...
                main_loss = self._loss_regrets[player](
                    regrets, preds, sample_weight=iterations * 2 / iteration
                )
    ...
    def _get_value_train_graph(self):
        ...
        def train_step(full_hist_states, values, iterations, masks, iteration):
            ...
            with tf.GradientTape() as tape:
                ...
                main_loss = self._loss_value(values, preds, sample_weight=1)
                loss = tf.add_n([main_loss], model.losses)
            gradients = tape.gradient(loss, model.trainable_variables)
    ...
    def _get_value_test_graph(self):
        ...
        def test_step(full_hist_states, values, iterations, masks, iteration):
            ...
            with tf.GradientTape():
                preds = model((full_hist_states, masks), training=True)
                main_loss = self._loss_value(values, preds, sample_weight=1)
                tf.add_n([main_loss], model.losses)
            return main_loss
    ...
    def _learn_value_network(self):
        ...
        self._val_network.set_weights(self._val_network_train.get_weights())
        return main_loss
...
if __name__ == "__main__":
    ...
    regret, pol_loss, convs, nodes = ray.get(
        deep_cfr_solver.solve.remote(save_path_convs=save_path)
    )
